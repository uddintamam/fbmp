...
Public Class PostFBPropertiControl
    ...
    Public Sub New(fiturPostFB As FiturPostFBEnum)
        ...
        Select Case _fiturPostFB
            Case FiturPostFBEnum.General
                lblHeader.Text = "FITUR AUTO POST FBMP PRODUK PROPERTI BIASA"
                'btnStartProcess.Text = "START ROBOT POST PRODUK PROPERTI BIASA"
            Case FiturPostFBEnum.OnlyDraft
                lblHeader.Text = "FITUR AUTO POST FBMP PRODUK PROPERTI DRAF"
                'btnStartProcess.Text = "START ROBOT POST PRODUK PROPERTI DRAF"
            Case FiturPostFBEnum.DraftAndPost
                lblHeader.Text = "FITUR AUTO POST FBMP PRODUK PROPERTI ANTI DUPLIKAT"
                'btnStartProcess.Text = "START ROBOT POST PRODUK PROPERTI ANTI DUPLIKAT"
        End Select
    ...
    Private Sub PostFBFurnitureControl_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region "Validasi sebelum menjalankan proses"
            If category < 0 Then
                MessageBox.Show("Rumah Dijual atau Dikontrakkan belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            ...
            If conditionProd < 0 Then
                MessageBox.Show("Jenis Properti belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If
            ...
            Try
                ...
                For i As Integer = nilaiAwal To nilaiAkhir
                    ...
                    Select Case _fiturPostFB
                        Case FiturPostFBEnum.General
                            postStatus =
                                    existingProfile.PostPropertiFB(jedarandom, fields, waitforelement,
                                                        categoryProd, conditionProd, Me.gridProfile, userId, current, jmlBaris, suspendEvent)
                        Case FiturPostFBEnum.OnlyDraft
                            postStatus =
                                    existingProfile.PostPropertiOnlyDraftFB(
                                        jedarandom, fields, waitforelement,
                                                    categoryProd, conditionProd, Me.gridProfile, userId, current, jmlBaris, suspendEvent)
                        Case FiturPostFBEnum.DraftAndPost
                            postStatus =
                                existingProfile.EditPropertiFromDraftFB(
                                    jedarandom, fields, waitforelement,
                                                categoryProd, conditionProd, Me.gridProfile, userId, current, jmlBaris, suspendEvent)
                            'End If
                    End Select
