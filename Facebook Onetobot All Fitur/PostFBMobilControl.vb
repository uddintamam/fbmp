...
Public Class PostFBMobilControl
    ...
    Public Sub New(fiturPostFB As FiturPostFBEnum)
        ...
        Select Case _fiturPostFB
            Case FiturPostFBEnum.General
                lblHeader.Text = "FITUR AUTO POST FBMP MOBIL BIASA"
                'btnStartProcess.Text = "START ROBOT POST MOBIL BIASA"
            Case FiturPostFBEnum.OnlyDraft
                lblHeader.Text = "FITUR AUTO POST FBMP MOBIL DRAF"
                'btnStartProcess.Text = "START ROBOT POST MOBIL DRAF"
            Case FiturPostFBEnum.DraftAndPost
                lblHeader.Text = "FITUR AUTO POST FBMP MOBIL ANTI DUPLIKAT"
                'btnStartProcess.Text = "START ROBOT POST MOBIL ANTI DUPLIKAT"
        End Select
    ...
    Private Sub PostFBMobilControl_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub btnStartProcess_Click(sender As Object, e As EventArgs) Handles btnStartProcess.Click
        ...
        If Not doubleClickOccurred Then
            ...
            Dim tahun As Integer = cbxTahun.SelectedIndex

#Region "Validasi sebelum menjalankan proses"
            If tahun < 0 Then
                MessageBox.Show("Tahun belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim merek As String = cbxMerek.Text

            If String.IsNullOrEmpty(merek) Then
                MessageBox.Show("Merek Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim bodi As Integer = cbxBody.SelectedIndex

            If bodi < 0 Then
                MessageBox.Show("Bodi Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim warna As Integer = cbxColor.SelectedIndex

            If warna < 0 Then
                MessageBox.Show("Warna Ekterior Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim warna2 As Integer = cbxColor2.SelectedIndex

            If warna2 < 0 Then
                MessageBox.Show("Warna Interior Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim conditionProd As Integer = cbxCondition.SelectedIndex

            If conditionProd < 0 Then
                MessageBox.Show("Kondisi Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim fuel As Integer = cbxFuel.SelectedIndex

            If fuel < 0 Then
                MessageBox.Show("Bahan Bakar Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If

            Dim transmission As Integer = cbxTransmission.SelectedIndex

            If transmission < 0 Then
                MessageBox.Show("Transmisi Mobil belum dipilih.", "Kesalahan", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                doubleClickOccurred = False
                Return
            End If
...
#End Region
            ...
            Try
                runRobot(cbxDataProfile.Text, tahun, merek, bodi, warna, warna2, conditionProd, fuel, chkTdkMasalah.Checked, transmission, chkRunAllProfile.Checked)
    ...
    Private Sub runRobot(profileName As String, tahun As Integer, merek As String, bodi As Integer, warna As Integer,
                          warna2 As Integer, conditionProd As Integer, fuel As Integer, tdkMasalah As Boolean,
                         transmission As Integer, isRunAll As Boolean)
            ...
            If existingProfile Is Nothing Then
                ...
                Dim newThread As Thread =
                    New Thread(Sub() runRobotWork(foundRows,
                                                   tahun, merek, bodi, warna, warna2, conditionProd, fuel, tdkMasalah, transmission,
                                                   isRunAll))
                '========================================
    ...
    Private Sub runRobotWork(foundRows As DataRow(), tahun As Integer, merek As String, bodi As Integer, warna As Integer,
                          warna2 As Integer, conditionProd As Integer, fuel As Integer, tdkMasalah As Boolean,
                             transmission As Integer, isRunAll As Boolean)
...
#Region "Proses membuka browser dan inject element ke dalam threads"
            ...
            If existingProfile Is Nothing Then
                If isRunAll Then
                    ...
                    Thread.Sleep(1000)
                Else
                    PostingLiteFb(foundRow, tahun, merek, bodi, warna, warna2, conditionProd, fuel, tdkMasalah, transmission,
                                                   fieldList)
                End If
            Else
                MessageBox.Show(
...
#End Region
    ...
    Private Sub PostingLiteFb(dataProfile As DataRow, tahun As Integer, merek As String, bodi As Integer,
                              warna As Integer, warna2 As Integer, conditionProd As Integer, fuel As Integer, tdkMasalah As Boolean, transmission As Integer,
            ...
            Try
                ...
                For i As Integer = nilaiAwal To nilaiAkhir
                    ...
                    Select Case _fiturPostFB
                        Case FiturPostFBEnum.General
                            postStatus =
                                    existingProfile.PostMobilFB(jedarandom, fields, waitforelement,
                                                        tahun, merek, bodi, warna, warna2, conditionProd, fuel, tdkMasalah, transmission,
                                                                Me.gridProfile, userId, current, jmlBaris, suspendEvent)
                        Case FiturPostFBEnum.OnlyDraft
                            postStatus =
                                    existingProfile.PostMobilOnlyDraftFB(jedarandom, fields, waitforelement,
                                                    tahun, merek, bodi, warna, warna2, conditionProd, fuel, tdkMasalah, transmission,
                                                    Me.gridProfile, userId, current, jmlBaris, suspendEvent)
                        Case FiturPostFBEnum.DraftAndPost
                            postStatus =
                                existingProfile.EditMobilFromDraftFB(jedarandom, fields, waitforelement,
                                                                    tahun, merek, bodi, warna, warna2, conditionProd, fuel, tdkMasalah, transmission,
                                                                        Me.gridProfile, userId, current, jmlBaris, suspendEvent)
                            'End If
                    End Select
                    ...
                    If postStatus IsNot Nothing Then
                        If postStatus.Status Then
                            gridReport.Invoke(
                                Sub() gridReport.Rows.Add("BERHASIL", fields(3),
                                                            runningProfile, userId, ""))
                        Else
                            gridReport.Invoke(
                                Sub() gridReport.Rows.Add("GAGAL", fields(3),
